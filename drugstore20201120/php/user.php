...
if (!$user || !$pass || !$type) {
  ...
  if ($type === 'login') {
    // 查询sql语句
    $login_sql = "select * from zhucee where phoned='$user' and password='$pass'";
    // 执行sql语句
  ...
  if ($type === 'add') {
    // 先查询注册的账号是否已存在
    $query_sql = "select * from zhucee where username='$user'";
    ...
    if (count($query_arr) > 0) {
      ...
      // 可以注册，插入数据
      $insert_sql = "insert into zhucee(phoned,password) values('$user','$pass')";
